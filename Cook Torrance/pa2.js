...
function updateSlider_PhongIOR(sliderAmount) {
    var value = sliderAmount/10.0;
    $("#sliderAmount_pIOR").html(value);

    gl.useProgram(shaderPrograms[5]);
    gl.uniform1f(shaderPrograms[5].iorUniform, value);
}

function updateSlider_PhongBeta(sliderAmount) {
    var value = sliderAmount/100.0;
    $("#sliderAmount_pBeta").html(value);

    gl.useProgram(shaderPrograms[5]);
    gl.uniform1f(shaderPrograms[5].betaUniform, value);
}

function updateSlider_BlinnPhongIOR(sliderAmount) {
    var value = sliderAmount/10.0;
    $("#sliderAmount_bpIOR").html(value);

    gl.useProgram(shaderPrograms[6]);
    gl.uniform1f(shaderPrograms[6].iorUniform, value);
}

function updateSlider_BlinnPhongBeta(sliderAmount) {
    var value = sliderAmount/100.0;
    $("#sliderAmount_bpBeta").html(value);
...
$(function() {
    ...
    //     change: function(color) {
    //         var color_ = color.toRgb();
    //         $("#albedoText").html(color.toHexString());
    //         albedoColor = [color_.r/255.0, color_.g/255.0, color_.b/255.0];
    //     }        
    // });
